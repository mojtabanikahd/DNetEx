...
from r_wrappers import SPDtrace, generate_reference_models
import numpy as np
import random
import pickle
import time
...
def get_metrics_values(metric_hist):
    ...
    return tpr_mean, fpr_mean, precision_mean, recall_mean, fdr_mean, q_mean, None, None, fdr_se, recall_se
...
def extract_metrics(metric_list):
    ...
    for i in range(len(metric_list)):
        ...
        tpr_hist_mean, fpr_hist_mean, precision_hist_mean, recall_hist_mean, fdr_hist_mean, qs_hist_mean, _, _, fdr_hist_se, recall_hist_se = get_metrics_values(metric_list_hist)
    ...
    metrics = [precision_list, recall_list, fdr_list, qs_list, _, _, fdr_list_se, recall_list_se]
    return metrics


def save_metric_files(metric_1, dir_name):
    with open(f"{dir_name}/our_method_metric_list", "wb") as fp:
...
labels = [f"{int(round(v * 10)):02d}" for v in r_list]
for r in r_list:
    dimension_metric_list = []
    dimension_metric_list_DNetFinder = []
    dir_name = f"Data/d_100_s_10_nr_100_rep_50_c_30_r_{int(round(r * 10)):02d}"
    ...
    for d in d_list:
        ...
        for i in range(rep):
            ...
            lasso_d1, lasso_d2, ols_d1, ols_d2 = split_dataset_samples(dataset_1, dataset_2, split_ratio=r)
    ...
    save_metric_files(our_metrics, dir_name)
